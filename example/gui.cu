...
void save_png(char* filename, vec4* image, size_t width, size_t height)
{
...
int main()
{
  ...
  testbed.frame();
  testbed.load_snapshot(static_cast<fs::path>(std::string(
      "/home/nghiatran/NERF/fns/build/_deps/ingp-src/data/nerf/fox.ingp")));
  ...
  for (int i = 0; i < 100; i++) {
    ...
    size_t frame_width =
        testbed.m_views.front().render_buffer.get()->in_resolution().x;
    size_t frame_height =
        testbed.m_views.front().render_buffer.get()->in_resolution().y;

    vec4* host_buffer =
        (vec4*)malloc(sizeof(vec4) * frame_width * frame_height);
